...
For the rest of the semester you will be given a `.Rmd` file like this to type your Problem set answers into. Please first be sure to put/save this file into your `introstatsR` folder so you can find it. 

Take a moment to knit the file, and you will see all the instructions for the problem set in the resulting `html` file. 

> You can type your answers right into this `.Rmd` document, knit the final copy, and turn in the **knitted `.html` file** as the answers to the problem sets. 
...
# Question 1
Take a look at the data in the data viewer. You can accomplish this by clicking on the name of the data in the Environment pane. What is the last city listed in the data set (in row 8602)?
...
# Question 5
Add a column **to the `dallas_sub` data set** called `prct_sold` that calculates the percentage of `listings` that were `sold` (`sales/listings * 1000`). Be sure to **save** the results also as a data frame called `dallas_sub`. 
...
# Question 7
Arrange the `dallas_summary` data frame in `desc`ending order based on the **average** percentage of listings sold, so you can see **which month** had the greatest **average** percentage of listings sold.  You do not need to save the results.  You do not need to save the results. 
