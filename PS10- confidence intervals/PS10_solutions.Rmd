...
$$\widehat{statistic} \pm 1.96 \cdot \widehat{SE}$$

So, for instance if we wanted to calculate the 95% confidence interval for the population mean number of `tvhours` $\mu_{tv}$ that respondents watched: 

* the **sample statistic** (also called **point estimate**) in this case would be the sample mean number of `tvhours` from the sample: $\bar{x}$
* We would estimate the standard error using the formula: where $s$ is the sample standard deviation of the sample:
...
Use the bootstrap resample we generated above (`boot_samp_1000`), to generate a **bootstrap distribution** for the sample mean respondent `age` $\bar{x}$ instead of `tvhours`. 
...
* the population parameter that we don't know, but are infering about via sampling, is now the population proportion $p$ that identify as a POC. 
