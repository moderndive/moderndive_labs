...
* `median_house_inc`: median household income in the state (low or high)
...
Super surprised. There must be another variable that is driving this. Or perhaps measuring at the unit level of the state is misleading giving the gerrymandering?
...
What are the two possible fitted values $\widehat{y}$ for this model? Why are there only two this time, instead of the three from the previous model?
...
Take a look at data for Maine. Did the model **overpredict** or **underpredict** the `hate_crimes` level, compared to what was observed in the data?
