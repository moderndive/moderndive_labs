...
**Answer: because every person took a different random sample with different people in it. In other words, sampling variation was at play.** 
...
$$SE_{\hat{p}} = \sqrt{\frac{\hat{p} \times (1-\hat{p})}{n}}$$
...
# Bert's Idea for a Question 11

Compare all the standard errors you computed for this problem set in the following R Markdown table. In other words:

1. Replace `x` with the standard error you obtained by taking the standard deviation of the sampling distribution you simulated for samples of size 15
2. Replace `a` with the standard error you obtained using the mathematical formula
3. ...

Sample size n  | SE via sd of sampling distribution | SE via formula
-------------- | ---------------------------------- | --------------
15             | x                                  | a
50             | y                                  | b
600            | z                                  | c
