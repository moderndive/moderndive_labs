...
Today we will practice data visualization using data on births from the state of North Carolina, stored in a data frame called `nc`, which is included in the `oilabs` package. So remember, before we can look at this data, we need to ensure the `oilabs` package is loaded. Load the data by running the following in a new code chunk.  
...
Because this is a data set built into an R package (`oilabs`), it has a help file describing all the variables. Type `?nc` into the **Console** (the pane on the bottom left), and hit `Enter`. Read through the help file. 
