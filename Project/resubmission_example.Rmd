...
high_schools_data_sat %>% 
  summarize(min(percent_economically_disadvantaged), 
            max(percent_economically_disadvantaged), 
            mean(percent_economically_disadvantaged), 
            sd(percent_economically_disadvantaged))
```
...
```{r include = F}
high_schools_data_sat %>%  filter(!is.na(score)) %>%
...
```{r}
temp <- high_schools_data_sat %>% filter(score != 741)

math_model2 <- lm(score~ percent_economically_disadvantaged + size, data = temp)
