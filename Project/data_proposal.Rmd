...
```{r, include=FALSE}
# This sets the options for the output of each code chunk is the resulting
# .html report:
...
# Why 76? For no other reason than I like this number. Why? See here:
...
The categorical explantory variable is called `size` and it has three levels: `small`, `medium`, and `large`. We constructed this variable by cutting the numeric variable `total_enrollment` into `n=3` roughly equally sized parts so that schools with 0-341 students were called `small`, schools with 342-541 students were classified as `medium` and those with > 541 students were `large`. 
...
There are 1861 rows in the original data set. However, we are limiting our analysis to only the 390 high schools (where 11th and 12th grade enrollment is greater than 0) as younger grades do not take the SAT. 
...
```{r}
ma_schools <- ma_schools %>% 
  select(school_name, average_sat_math, percent_economically_disadvantaged, size)
```
