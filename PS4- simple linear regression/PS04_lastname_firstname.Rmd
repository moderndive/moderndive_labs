...
We will read the data in with the `readr` package, explore the data using the `dplyr` package and visualize it using the `ggplot2` package. The `moderndive` package includes some nice functions to show us outputs relating to regression model. 
...
We'll do this by creating a scatterplot with `trump_support` as the outcome variable on the y-axis and `poverty` as the explanatory variable on the x-axis. Note the use of the `geom_smooth()` function, that tells R to add a regression line. 
...
Make a numerical prediction for the level of trump support for a region that has 70% of humans that identify as a person of color. In other words, use **math** not a visual prediction below:
...
If Representative Ocasio-Cortez saw the regression line and not the actual data, what would her prediction of Trump support be for a region with 50% of people that identify as non-white? Would her prediction be an overestimate or an underestimate (compared to the observed data), and by how much? In other words, what is the residual for this prediction? **Hint:** use the `get_regression_points()` function!
